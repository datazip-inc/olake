...
func TestValidateCatalogFlags(t *testing.T) {
	testCases := []struct {
		...
		{name: "legacy and available only", streams: "streams.json", available: "available_streams.json", required: true, expectErr: true, expectedCode: codeConflictingStreamFlags},
	}
