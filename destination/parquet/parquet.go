...
func (p *Parquet) clearS3Files(ctx context.Context, selectedStreams []string) error {
	...
	for _, streamID := range selectedStreams {
		...
		// Construct S3 path consistently with the writing logic
		// Use the same basePath construction as in writing logic
		basePath := filepath.Join(namespace, streamName)
		if p.config.Prefix != "" {
			basePath = filepath.Join(p.config.Prefix, basePath)
		}
		// Convert to S3-compatible path (forward slashes)
		s3TablePath := strings.ReplaceAll(basePath, "\\", "/")
