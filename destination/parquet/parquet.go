...
func (p *Parquet) Setup(_ context.Context, stream types.StreamInterface, schema any, options *destination.Options) (any, error) {
	...
	p.basePath = filepath.Join(utils.Ternary(p.stream.GetDestinationDatabase() != "", p.stream.GetDestinationDatabase(), p.stream.Namespace()).(string), utils.Ternary(p.stream.GetDestinationTable() != "", p.stream.GetDestinationTable(), p.stream.Name()).(string))
