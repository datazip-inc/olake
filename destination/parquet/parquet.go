...
func (p *Parquet) Setup(_ context.Context, stream types.StreamInterface, schema any, options *destination.Options) (any, string, error) {
	p.options = options
	p.stream = stream
...
func (p *Parquet) closePqFiles(ctx context.Context, _ string, closeOnError bool) error {
...
func (p *Parquet) Close(ctx context.Context, metadataState string) error {
	// TODO: implement 2pc in parquet writer (difficulty: hard)
	return p.closePqFiles(ctx, metadataState, ctx.Err() != nil)
}
