	...
	case "insert":
		query = fmt.Sprintf(`
			INSERT INTO %s (
				...
				'2023-01-01 12:00:00.573605', '2023-01-01 12:00:00.573605+00',
	...
	case "update":
		query = fmt.Sprintf(`
			UPDATE %s SET
				...
				col_timestamp = '2024-07-01 15:30:00.573605',
				col_timestamptz = '2024-07-01 15:30:00.573605+00',
...
func insertTestData(ctx context.Context, t *testing.T, db *sqlx.DB, tableName string) {
	...
	for i := 1; i <= 5; i++ {
		...
		INSERT INTO %s (
			...
			'sample text', '2023-01-01 12:00:00.573605',
			'2023-01-01 12:00:00.573605+00',
			'123e4567-e89b-12d3-a456-426614174000', B'101010',
			'<tag>value</tag>',
			'(10.5,20.5)'::point,
			'((0,0),(10,0),(10,10),(0,10),(0,0))'::polygon,
...
var ExpectedUpdatedData = map[string]interface{}{
	...
	"col_text":              "updated text",
	"col_timestamp":         arrow.Timestamp(time.Date(2024, 7, 1, 15, 30, 0, 573605000, time.UTC).UnixNano() / int64(time.Microsecond)),
	"col_timestamptz":       arrow.Timestamp(time.Date(2024, 7, 1, 15, 30, 0, 573605000, time.UTC).UnixNano() / int64(time.Microsecond)),
	"col_uuid":              "00000000-0000-0000-0000-000000000000",
	"col_varbit":            "111000",
	"col_xml":               "<updated>value</updated>",
	"col_point":             "(15.5,25.5)",
