	...
	case "create":
		...
		query = fmt.Sprintf(`
			CREATE TABLE IF NOT EXISTS %[1]s (
				col_bigint BIGINT,
				col_cursor INT,
				col_bigserial BIGINT GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME seq_%[2]s),
				col_bool BOOLEAN,
				col_char CHAR(1),
				...
				col_polygon POLYGON,
				col_circle CIRCLE,
				CONSTRAINT pkey_%[2]s PRIMARY KEY (col_bigserial),
				CONSTRAINT key_%[2]s UNIQUE (col_bigserial),
				excludedColumn INT NULL
			)`, integrationTestTable, id)
