...
func (cfg *Test) TestDiscover(t *testing.T) {
	...
	code, out, err := testutils.RunOlake(ctx, t, cfg.TestConfig, testutils.DiscoverArgs()...)
	if err != nil || code != 0 {
		t.Fatalf("discover failed (%d): %s\n%s", code, err, string(out))
	}

	verifyDiscoveredStreams(t, cfg.GetFilePath("test_streams.json"), cfg.GetFilePath("streams.json"))
}
