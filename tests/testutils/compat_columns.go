...
// CompatColumnRule is one column's backward-compat assertion policy, keyed on the baseline release
...
// green run proves.
func resolveColumnPolicies(rules []CompatColumnRule, spec string) (*columnPolicies, error) {
	version, dated := parseReleaseTag(spec)
	policies := &columnPolicies{}
	seen := make(map[string]bool, len(rules))
	for _, rule := range rules {
		if rule.Column == "" {
			return nil, fmt.Errorf("compat column rule with an empty column name: %+v", rule)
		}
		if seen[rule.Column] {
			return nil, fmt.Errorf("duplicate compat column rule for %q; one rule carries every policy for a column", rule.Column)
		}
		seen[rule.Column] = true
		if rule.ExcludeBelow == "" && rule.AssertValueFrom == "" && !rule.TypeOnly {
			return nil, fmt.Errorf("compat column rule for %q declares no policy", rule.Column)
		}
		if rule.TypeOnly && rule.AssertValueFrom != "" {
			return nil, fmt.Errorf("compat column rule for %q sets both TypeOnly and AssertValueFrom, which contradict", rule.Column)
		}

		if rule.ExcludeBelow != "" {
			boundary, ok := parseReleaseTag(rule.ExcludeBelow)
			if !ok {
				return nil, fmt.Errorf("compat column rule for %q: ExcludeBelow %q is not a release tag", rule.Column, rule.ExcludeBelow)
			}
		...
		case rule.AssertValueFrom != "":
			...
			if !ok {
				return nil, fmt.Errorf("compat column rule for %q: AssertValueFrom %q is not a release tag", rule.Column, rule.AssertValueFrom)
			}
			if dated && compareRelease(version, boundary) < 0 {
				policies.typeOnly = append(policies.typeOnly, rule.Column)
				policies.notes = append(policies.notes, fmt.Sprintf(
