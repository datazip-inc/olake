...
// renders it and seedColumnTypes reads it.
const seedTableDDL = `
...
// seedColumnTypes derives every seed column's type tags from the DDL, so a data_types rule in
// compatibility_rules.json follows a seed edit with nothing to declare.
func seedColumnTypes() map[string][]string {
	return testutils.DDLColumnTypes(seedTableDDL)
}
...
func ExecuteQuery(ctx context.Context, t *testing.T, conf *testutils.TestConfig, operation string) {
	...
	case "create":
		query = fmt.Sprintf("CREATE TABLE %s (%s)", integrationTestTable, seedTableDDL)
