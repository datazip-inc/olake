...
func (c *Config) Validate() error {
	...
	if c.JarPath == "" {
		...
		if err := utils.CheckIfFilesExists(baseJarPath); err == nil {
			...
			targetJarPath := fmt.Sprintf("%s/writers/iceberg/olake-iceberg-java-writer/target/olake-iceberg-java-writer-0.0.1-SNAPSHOT.jar", execDir)
			if err := utils.CheckIfFilesExists(targetJarPath); err == nil {
				...
				return fmt.Errorf("Iceberg JAR file not found in any of the expected locations: %s, %s. Go to writers/iceberg/olake-iceberg-java-writer/target/ directory and run mvn clean package -DskipTests",
