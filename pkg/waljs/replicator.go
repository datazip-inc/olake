...
// AdvanceLSN moves the slot to currentWalPos, retrying until postgres confirms it there, and
// returns the confirmed position: pg_replication_slot_advance silently clamps to the flushed WAL.
func AdvanceLSN(ctx context.Context, db *sqlx.DB, slot, currentWalPos string) (pglogrepl.LSN, error) {
	target, err := pglogrepl.ParseLSN(currentWalPos)
	if err != nil {
		return 0, fmt.Errorf("failed to parse advance target lsn[%s]: %s", currentWalPos, err)
	}
	...
	for {
		...
		if err := db.QueryRowContext(ctx, fmt.Sprintf(AdvanceLSNTemplate, slot, currentWalPos)).Scan(&endLSN); err != nil {
			return 0, fmt.Errorf("failed to advance replication slot: %w", err)
		}
		if endLSN >= target {
			logger.Debugf("advanced LSN to %s", endLSN)
			return endLSN, nil
		}
		...
		case <-timeoutCtx.Done():
			return 0, fmt.Errorf("slot advance stopped at %s and could not reach target %s: %w", endLSN, target, timeoutCtx.Err())
