...
const (
	...
	installCmd            = "apt-get update && apt-get install -y openjdk-17-jre-headless maven default-mysql-client iproute2 dnsutils iputils-ping netcat-openbsd nodejs npm jq && npm install -g chalk-cli"
)
...
func TestMySQLIntegration(t *testing.T) {
	...
	testdataDir := filepath.Join(projectRoot, "drivers/mysql/internal/testdata")
		...
		req := testcontainers.ContainerRequest{
			...
			HostConfigModifier: func(hc *container.HostConfig) {
				hc.Binds = []string{
					...
					fmt.Sprintf("%s:/test-olake/drivers/mysql/internal/testdata:rw", testdataDir),
				}
			...
			LifecycleHooks: []testcontainers.ContainerLifecycleHooks{
				{
					PostReadies: []testcontainers.ContainerHook{
						func(ctx context.Context, c testcontainers.Container) error {
							...
							t.Logf("MySQL discover test-container clean up")
							return nil
						},
					},
				},
					...
					PostReadies: []testcontainers.ContainerHook{
						func(ctx context.Context, c testcontainers.Container) error {
							...
							streamUpdateCmd := fmt.Sprintf(
								`jq '.selected_streams.olake_mysql_test[] .normalization = true' %s > /tmp/streams.json && mv /tmp/streams.json %s`,
							...
							testCases := []struct {
								...
								},
								{
									...
									useState:    true,
									opSymbol:    "u",
									dummySchema: ExpectedUpdatedMySQLData,
								},
								{
									syncMode:    "CDC - delete",
									operation:   "delete",
							...
							runSync := func(c testcontainers.Container, useState bool, operation, opSymbol string, schema map[string]interface{}) error {
								...
								if useState {
									...
									cmd = fmt.Sprintf("/test-olake/build.sh driver-mysql sync --config %s --catalog %s --destination %s --state %s", sourceConfigPath, streamsPath, destinationConfigPath, statePath)
								} else {
									cmd = fmt.Sprintf("/test-olake/build.sh driver-mysql sync --config %s --catalog %s --destination %s", sourceConfigPath, streamsPath, destinationConfigPath)
								}
