...
const chunkSize int64 = 500000 // Default chunk size for Oracle

func (o *Oracle) ChunkIterator(ctx context.Context, stream types.StreamInterface, chunk types.Chunk, OnMessage abstract.BackfillMsgFn) (err error) {
	// Begin transaction with read committed isolation (Oracle default)
	return jdbc.WithIsolation(ctx, o.client, func(tx *sql.Tx) error {
		...
		stmt := utils.Ternary(chunkColumn != "", jdbc.OracleChunkScanQuery(stream, chunkColumn, chunk), utils.Ternary(len(pkColumns) > 0, jdbc.OracleChunkScanQuery(stream, strings.Join(pkColumns, ", "), chunk), jdbc.OracleWithoutState(stream))).(string)
		...
		return setter.Capture(func(rows *sql.Rows) error {
			...
			err := jdbc.MapScan(rows, record, o.dataTypeConverter)
			if err != nil {
...
func (o *Oracle) GetOrSplitChunks(ctx context.Context, pool *destination.WriterPool, stream types.StreamInterface) (*types.Set[types.Chunk], error) {
	...
	approxRowCountQuery := jdbc.OracleTableRowsQuery()
	err := o.client.QueryRow(approxRowCountQuery, stream.Name(), stream.Namespace()).Scan(&approxRowCount)
	if err != nil {
	...
	splitViaPrimaryKey := func(stream types.StreamInterface, chunks *types.Set[types.Chunk]) error {
		return jdbc.WithIsolation(ctx, o.client, func(tx *sql.Tx) error {
			...
			// Get table extremes
			minVal, maxVal, err := o.getTableExtremes(stream, pkColumns, tx)
			if err != nil {
...
func (o *Oracle) getTableExtremes(stream types.StreamInterface, pkColumns []string, tx *sql.Tx) (min, max any, err error) {
	// Oracle-specific min/max query
	query := fmt.Sprintf(`
		SELECT 
			(SELECT %s FROM %s.%s ORDER BY %s FETCH FIRST 1 ROW ONLY) AS min_value,
			(SELECT %s FROM %s.%s ORDER BY %s DESC FETCH FIRST 1 ROW ONLY) AS max_value
		`,
		strings.Join(pkColumns, ", "), stream.Namespace(), stream.Name(), strings.Join(pkColumns, ", "),
		strings.Join(pkColumns, ", "), stream.Namespace(), stream.Name(), strings.Join(pkColumns, ", "))
