...
type DriverInterface interface {
	...
	// specific to cdc
	CDCSupported() bool
	// CDCMetadataColumns returns driver-specific CDC metadata columns (name -> type).
	// Return nil when the driver has none (Kafka, S3, DB2, Oracle).
	CDCMetadataColumns() map[string]types.DataType
