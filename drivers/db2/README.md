...
## Supported Modes
1. **Full Refresh**
   Fetches the complete dataset from DB2 LUW.
2. **Incremental**
...
To run the DB2 LUW Driver, configure the following files with your specific credentials and settings:

- **`config.json`**: DB2 LUW connection details.
...
### Config File
Add DB2 LUW credentials in following format in `config.json` file. [More details.](https://olake.io/docs/connectors/DB2 LUW/config)
   ```json
  {
    "host": "DB2 LUW-host",
    "port": 50000,
...
To run the Discover command, use the following syntax
   ```bash
   ./build.sh driver-db2 discover --config /db2/examples/config.json
   ```
...
### Sync Command
The *Sync* command fetches data from DB2 LUW and ingests it into the destination.

- Without State
```bash
./build.sh driver-db2 sync --config /db2/examples/config.json --catalog /db2/examples/streams.json --destination /db2/examples/write.json
```

- With State
```bash
./build.sh driver-db2 sync --config /db2/examples/config.json --catalog /db2/examples/streams.json --destination /db2/examples/write.json --state /db2/examples/state.json
```

Find more at [DB2 LUW Docs](https://olake.io/docs/category/db2)