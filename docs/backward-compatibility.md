...
`LoadedStateVersion` is the only mechanism that enforces the first half of that sentence. It does not cover everything a user hits when they upgrade — §2 enumerates what it covers and what it does not, and §3 says when any of it actually matters.
...
| --- | --- | --- |
| `LatestStateVersion` | `constants/state_version.go:42` | the version a *new* pipeline is created at (currently `6`) |
| `LatestStateVersion` | `tests/testutils/constants/constants.go:33` | the test harness's deliberately separate copy — a black-box suite sharing the product's symbol could not detect a contract break |
| `LoadedStateVersion` | `constants/state_version.go:45` | the version the *current process* is running as |

`LoadedStateVersion` is assigned in exactly one place:
...
Six sites read `LoadedStateVersion`. They live in three different Go modules — root (`github.com/datazip-inc/olake`), `drivers/mysql` and `drivers/mongodb` — which is why the unit tests for them cannot live in one place.
...
Needed when picking a baseline image to test an upgrade against. Verified by reading `constants/state_version.go` at every released tag.
...
| 6 | #797 `[]uint8` in `ReformatInt64` | v0.7.0 | v0.9.0 (current) |

The "newest release still on it" column is load-bearing: those tags are `COMPAT_SWEEP_BASELINES` in the Makefile, the versions the compat sweep fans out to on every merge to master. **Bumping `LatestStateVersion` means appending the newest release still on the old version to that list** — the constant carries a reminder comment.
...
for v := 0; v <= constants.LatestStateVersion; v++ {
    exp, ok := want[v]
...
`withStateVersion` is one shared helper that sets `constants.LoadedStateVersion` and restores it via `t.Cleanup`; the idiom is already inlined twice in `utils/typeutils/reformat_test.go` (`:1270-1276` and `:1583-1589`) and should be extracted rather than copied again.
...
- for every version N ≥ 1, at least one `LoadedStateVersion` comparison exists somewhere in the tree that distinguishes N-1 from N.
...
### Layer 3 — Real old-image → new-image upgrade suite (implemented: `tests/testutils/compat.go`)

- **Catches:** I1, I3, I5, I6 — the breaks that only appear when a binary meets a state file and a table it did not create.
- **Cannot catch:** gates for versions older than the oldest baseline in the sweep; anything on a driver without a `Test<Driver>Compat` entry point (`COMPAT_DRIVERS` in the Makefile).

Rather than per-version expectations — which rot — `RunBackwardCompat` runs one driver's six scenarios (iceberg legacy/arrow/parquet × CDC/incremental) twice, concurrently, and asserts the two destinations are indistinguishable:
...
**Inputs are era-correct** (`compat_input.go`). Both sides get the `streams.json` shape and CLI flags the *baseline* shipped with — `selected_columns` (v0.4.0), `filter_config` vs the legacy `filter` string (v0.6.0), `--destination-database-prefix` (v0.2.0). A key the baseline never understood would otherwise read as a behaviour change. `OLAKE_COMPAT_INPUT_GENERATION` pins a shape by hand.

**Per-column assertion policy** (`compat_columns.go`). Known, documented findings are encoded as `CompatColumnRule` entries on each driver's compat config, resolved against the baseline's release tag:
...
**Baselines.** `COMPAT_BASELINE` (make) / `OLAKE_COMPAT_BASELINE` (env, `_<DRIVER>` suffix per driver) accepts a release tag, a full image ref, or a commit sha — the sha path builds the baseline from a detached worktree. Default: the newest release. The sweep list `COMPAT_SWEEP_BASELINES` is the newest release still on each state version (the table in §1) plus `latest`, and every entry must stay ≥ v0.3.17 — below that the shared Iceberg writer had the P1/P2 window (COMPAT_RESULTS_v2.md), which is a variant-level failure no column rule can express.

**CI cadence** — three triggers, two workflows:

- **Merge to master** (`compat-tests.yml`): the full sweep, `COMPAT_DRIVERS` × `COMPAT_SWEEP_BASELINES`, one job each. `OLAKE_COMPAT_REQUIRE_BASELINE` is set, so an unresolvable baseline is a red run, never a silent skip.
